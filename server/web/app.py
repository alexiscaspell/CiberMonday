...
# ADMIN_ALLOW_LAN=true permite toda la LAN privada (útil en cibercafé / Docker)
...
ADMIN_ALLOW_LAN = os.getenv('ADMIN_ALLOW_LAN', 'false').strip().lower() in ('1', 'true', 'yes', 'on')


def _is_private_ip(addr: str) -> bool:
    """True para RFC1918 / link-local IPv4 (LAN y bridge Docker)."""
...
def _is_admin_request():
    ...
    if addr in ADMIN_ALLOWED_IPS:
        return True
    # Acceso al contenedor vía puertos publicados: Flask ve la IP del bridge Docker
    if _is_private_ip(addr) and (ADMIN_ALLOW_LAN or addr.startswith('172.')):
        return True
    return False
